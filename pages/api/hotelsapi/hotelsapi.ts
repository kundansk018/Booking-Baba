...
export async function addhotel(request: NextApiRequest, response: NextApiResponse) {
    const hotels = await db.collection("Hotels_Details");
    const res = await hotels.insertOne(request.body);
    return response.status(200).json({ data: res });
}

export async function gethotel(request: NextApiRequest, response: NextApiResponse) {
    const hotels = await db.collection("Hotels_Details");
